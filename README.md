...
- App APIから取得するネイティブコメント一覧、返信元表示、無限スクロール、投稿者プロフィール遷移
...
通常ビルドではAPKとSHA-256をActions Artifactへ30日間保存する。`v2.0.1`のようなタグをpushすると、同じ成果物をGitHub Releaseへ自動添付する。Actions画面から手動実行するときは`publish_release`をオンにすれば、`app.json`のバージョン名でタグとReleaseを作成する。

内部APIの確認済み仕様と実装場所は[`docs/pixiv-api-notes.md`](./docs/pixiv-api-notes.md)へ記録する。
