...
- ブックマーク追加・解除
- 作品詳細を通常Routeで表示し、「最初から読む」は読書Routeへ直接置換
- 読書画面を閉じると詳細を挟まず一覧へ戻るナビゲーション
...
通常ビルドではAPKとSHA-256をActions Artifactへ30日間保存する。`v1.6.0`のようなタグをpushすると、同じ成果物をGitHub Releaseへ自動添付する。
